...
function(X, format = c("probability.different", "percent.same", "general"), compute.all = FALSE, check.computation = FALSE){
	...
	#               regular minimality (otherwise function fechner stops with respective error messages); in particular, data
	...
	# generalized "Shepardian" dissimilarity (or S-index); it is defined as the psychometric length of the loop
	...
	if (!compute.all){
		...
		# matrix of the graph-theoretic (edge-based) lengths of the shortest paths from source vertices (stimuli) to target vertices (stimuli);
		...
		# matrices of the graph-theoretic (edge-based) lengths of the shortest paths from source vertices (stimuli) to target vertices (stimuli);
