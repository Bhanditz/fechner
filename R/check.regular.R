	...
	if (check.reg.minimal){  # checks regular minimality
		for (i in 1:dim(M)[1]){
			...
			if (length(index.row) > 1) 
				stop(paste("regular minimality is violated: more than one minimal entry found in row #", i, sep = ""))
			index.col <- which(M[, index.row] == min(M[, index.row]))
			if (length(index.col) > 1) 
				stop(paste("regular minimality is violated: more than one minimal entry found in column #", index.row, sep = ""))
			if (index.col != i) 
				stop(paste("regular minimality is violated: entry in row #", i," and column #", index.row, 
		...
		for (i in 1:dim(M)[1]){
			...
			if (length(index.row) > 1) 
				stop(paste("regular maximality is violated: more than one maximal entry found in row #", i, sep = ""))
			index.col <- which(M[, index.row] == max(M[, index.row]))
			if (length(index.col) > 1) 
				stop(paste("regular maximality is violated: more than one maximal entry found in column #", index.row, sep = ""))
			if (index.col != i) 
				stop(paste("regular maximality is violated: entry in row #", i," and column #", index.row,
